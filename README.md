...
2. **Phone app**
   ...
   - A pending mobile follow-up will add `APPWRITE_GET_MOUNTED_ROBOT_FUNCTION_ID`, point `APPWRITE_CLAIM_ROBOT_FUNCTION_ID` at a separate/versioned secure Function for supported Android Robot builds, and restore through `get-mounted-robot`. Until it is released, configured, and validated, legacy clients stay on the legacy Function. The target contract is seven server-authorized Functions and six server-only TablesDB tables. Caregiver medication sync is an inactive source foundation: medication schedules, dose state/history, inventory, and related medication data remain in local Drift/SQLite, with no active upload or sync.
